...
class AnchorFreeSingle(nn.Module):
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds):
        ...
        stride =  post_process_cfg.get('STRIDE',2)
        post_process_style = post_process_cfg.get('POST_PROCESS_STYLE', 'anchor_free') 
        pre_max_size = post_process_cfg.get('PRE_MAX_SIZE',100)
        ...
        if post_process_style == 'anchor_free':
            ret_dict = postprocessing_v2(cls_preds, box_preds, self.point_cloud_range, self.grid_size, self.num_class, stride, score_thresh, pre_max_size, ret_dict,post_process_cfg)
...
def postprocessing_v2(cls_preds, box_preds, point_cloud_range, grid_size, num_class, stride, score_thresh, pre_max_size, ret, post_process_cfg=None):
    ...
    for i in range(detections.shape[0]):
        ...
        if post_process_cfg is not None:
            cls_preds = detection[:, 0].contiguous()
            label_preds = detection[:, -1].int() + 1 # 从1开始
            box_preds = detection[:, 1:8].contiguous()
            ...
            ret[i]["pred_boxes"] = final_boxes
            ret[i]["pred_scores"] = final_scores
            ret[i]["pred_labels"] = final_labels
        else:
