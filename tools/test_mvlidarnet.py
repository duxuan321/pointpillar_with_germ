...
from eval_utils import eval_utils_after
...
def eval_single_ckpt1(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False):
    # start evaluation
    eval_utils_after.eval_one_epoch(
        cfg, model, test_loader, epoch_id, logger, dist_test=dist_test,
        result_dir=eval_output_dir, save_to_file=args.save_to_file
    )
...
def main():
    ...
    for i, data in enumerate(test_loader):
        ...
        # import pdb;
        # pdb.set_trace()
        datalist.append(torch.from_numpy(data['bev_map']))

    input_vars = [torch.randn((1, 3, 432, 496))]
    work_dir = '/home/yuanxin/mvlidarnet20220728'
    onnx_model = onnx.load('/home/yuanxin/mvlidarnet20220728/mvlidarnet_epoch_80_default.onnx')
    quant_model = run_quantizer(onnx_model, dataloader=datalist, num_batches=2,
