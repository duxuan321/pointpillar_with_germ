...
case_name = 'mvlidarnet'
work_dir = 'regression_test_tempfiles/' + case_name
if os.path.exists(work_dir):
    shutil.rmtree(work_dir)
os.makedirs(work_dir, exist_ok=True)

input_shape = (1, 3, 704, 800)
    ...
    with torch.no_grad():
        ...
        if args.image_input:
            ...
            for i, data in enumerate(test_loader):
                ...
                import pdb;
                pdb.set_trace()
                datalist.append((torch.from_numpy(data['bev_map']), 1))

            val_loader = datalist
            print('val')
            print('val_loader', val_loader)
            input_vars = [torch.randn((1, 3, 432, 496))]
            onnx_model = onnx.load(
                '/home/yuanxin/mvlidarnet_pcdet/output/cfgs/kitti_models/mvlidarnet/default/mvlidarnet_epoch_80_default.onnx')
            # list_shape = np.array(data['img']).shape
            # print('list shape',list_shape)
            # print('datalist',data['img'].shape)
            import pdb;
            pdb.set_trace()
            # If there is a problem, please find @超星未来-郑成林 to solve it.
            run_quantizer(onnx_model, val_loader, num_batchs=1, save_dir=work_dir + '/ir_output', input_vars=input_vars)
