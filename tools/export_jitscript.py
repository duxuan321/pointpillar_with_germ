...
class ExportModel(nn.Module):
    ...
    def forward(self, bev_map):
        batch_dict = {'spatial_features': bev_map}

        global data_dict_info
        data_dict_info.update(batch_dict)
        data_dict_info = self.backbone_2d(data_dict_info)
        data_dict_info = self.dense_head(data_dict_info)

        cls_preds = data_dict_info["export_cls_preds"]
        box_preds = data_dict_info["export_box_preds"]

        return torch.cat([cls_preds,box_preds],1)
...
def main():
    ...
    try:
        pt_name = args.ckpt[args.ckpt.rindex('/')+1:args.ckpt.rindex('.')] + '_' + args.extra_tag + '.pt'
    except:
        pt_name = 'deploy.pt'
    pt_path = os.path.join(onnx_dir, pt_name)
    ...
    with torch.no_grad():
        export_model.eval()
        script_model = torch.jit.trace(export_model, bev_map)
        script_model.save(pt_path)
        print("导出pt成功")
